...
# Configuration
NTFY_SH_BASE_URL = "https://ntfy.sh"
REDIS_HOST = "localhost"
REDIS_PORT = 6379
REDIS_DB = 0
REDIS_NOTIFICATION_PREFIX = "notifications:ratelimit:"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/var/log/sentient/notifications.log')
    ]
)
...
class NotificationService:
    ...
    def __init__(self, ntfy_topic_url: Optional[str] = None):
        ...
        Args:
            ntfy_topic_url: Full URL for ntfy.sh topic (e.g., https://ntfy.sh/my-sentient-core)
                           If None, will attempt to load from Redis config
        """
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Load or set ntfy topic URL from Redis if not provided
            if not self.ntfy_topic_url:
                stored_url = await self.redis_client.get("config:ntfy:topic_url")
                if stored_url:
                    self.ntfy_topic_url = stored_url
                    logger.info(f"Loaded ntfy topic URL from Redis: {self.ntfy_topic_url}")
                else:
                    logger.warning("No ntfy topic URL configured. Set via set_config() or environment.")
    ...
    async def _check_rate_limit(self, priority: PriorityLevel) -> bool:
        ...
        try:
            ...
            rate_key = f"{REDIS_NOTIFICATION_PREFIX}{priority.name}:{hour_key}"
            ...
            logger.debug(f"Rate limit OK for {priority.name}: {current}/{limit}")
            return True
...
async def main():
    ...
    try:
        # Initialize service
        # In production, you'd pass the actual ntfy.sh topic URL here
        # or rely on configuration in Redis
        logger.info("Initializing notification service...")

        _service = NotificationService()

        # For testing, you can set the topic URL directly
        # _service.ntfy_topic_url = "https://ntfy.sh/my-sentient-core-test"
        ...
        logger.info("Set ntfy topic URL with: service.set_config('ntfy:topic_url', 'https://ntfy.sh/your-topic')")
        logger.info("Send notification with: await service.send_notification('Test message')")
