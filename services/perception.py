...
class PerceptionLayer:
    ...
    def __init__(
        ...
        logger.info(f"PerceptionLayer initialized for {mqtt_broker}:{mqtt_port}")
        ...
        while self.running:
            try:
                async with aiomqtt.Client(**mqtt_params) as client:
                    ...
                    await client.subscribe("sentient/sensor/rf/detection")
                    await client.subscribe("sentient/system/status")
                    ...
                    async for message in client.messages:
                        try:
                            ...
                            elif topic == "sentient/sensor/rf/detection":
                                await self.handle_rf_detection(topic, payload)
                            elif topic == "sentient/system/status":
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Sentient Core Perception Layer")
    parser.add_argument(
        "--broker",
        default="localhost",
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="World state publish interval (seconds)"
    )
